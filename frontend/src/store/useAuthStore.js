...
import { USER_API_END_POINT } from "../lib/axios.js";
...
export const useAuthStore = create((set, get) => ({
  ...
  checkAuth: async () => {
    try {
      ...
      const res = await axios.get(`${USER_API_END_POINT}/check`, {
...
//error yahi pe aayega agar aana ho toh
  connectSocket: () => {
    const { authUser } = get();
    if (!authUser || get().socket?.connected) return;

    const socket = io("https://chat-app-aman.onrender.com", {
      query: {
        userId: authUser._id,
      },
    });
